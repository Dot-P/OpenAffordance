...
def confusion_matrix(pred_ids, gt_ids, num_classes):
    ...
    # print("[debug] pred min/max:", int(pred_ids.min()), int(pred_ids.max()))
    # print("[debug]  gt  min/max:", int(gt_ids.min()),   int(gt_ids.max()))
...
def evaluate(pred_ids, gt_ids, stdout=False, dataset='scannet_3d'):
    ...
    elif 'openscan_3d' in dataset:
        CLASS_LABELS = OPENSCAN_AFFORDANCE_LABELS_105
    elif 'scenefun_3d' in dataset:
        CLASS_LABELS = SCENEFUN_LABELS_9
    else:
