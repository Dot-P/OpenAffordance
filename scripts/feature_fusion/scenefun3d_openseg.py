...
def _load_points_from_pth(path: str) -> torch.Tensor:
    """Load point coordinates tensor from a .pth file (minimal, SceneFun3D/OpenScene dict format)."""
    obj = torch.load(path, map_location="cpu")
    if isinstance(obj, dict):
        if "coords" in obj and torch.is_tensor(obj["coords"]):
            return obj["coords"].float()
        # フォールバック（極力使わない）
        for k in ("locs", "locs_in", "xyz", "points", "pos"):
            if k in obj and torch.is_tensor(obj[k]):
                return obj[k].float()
        for v in obj.values():
            if torch.is_tensor(v) and v.ndim >= 2 and v.shape[1] >= 3:
                return v[:, :3].float()
        raise KeyError(f"No point tensor found in dict keys={list(obj.keys())}")
...
def process_one_scene(data_path, out_dir, args):
    ...
    locs_in = _load_points_from_pth(data_path)
    ...
    img_dirs = sorted(glob(join(scene, 'color/*')), key=lambda x: int(os.path.basename(x)[:-4]) if os.path.basename(x)[:-4].isdigit() else os.path.basename(x)[:-4])
...
def main(args):
    ...
    # ==== 最小変更：SceneFun3D(OpenScene) 構造に合わせる ====
    data_root = join(data_dir, 'scenefun_3d')
    data_root_2d = join(data_dir,'scenefun_2d')
